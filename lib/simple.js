 ...
 */
    ...
    */
    conn.sendTB = async (jid, contentText, footer, dtux, urlx, dtcx, nmbrx, buttons1, row1, buttons2, row2, buttons3, row3, quoted, options) => {
        ...
        /**
    * send Template Button with url and 2 button
    * @param {String} jid 
    * @param {String} contentText 
    * @param {String} footer
    * @param {String} buttons
    ...
    * @param {Object} quoted 
    */
         conn.sendTB2 = async (jid, contentText, footer, dtux, urlx,buttons1, row1, buttons2, row2, quoted, options) => {
            const message = {
                ...options,
                text: contentText,
                footer: footer,
...
/**
     ...
     * @param {Object} options 
     */
 conn.sendTBD2 = async (jid, contentText, footer, thumb, dtux, urlx,buttons1, row1, buttons2, row2,quoted, options) => {
    const message = {
        document: { url: docux }, ...options,
        jpegThumbnail: await (await fetch(thumb)).buffer(), fileName: global.nd, mimetype: global.td, fileLength: global.fsdx, pageCount: global.pcdx,
        caption: contentText,
  ...
  * @param {Object} options 
  */
    conn.sendTBI = async (jid, contentText, footer, image, dtux, urlx, buttons1, row1, buttons2, row2, buttons3, row3, quoted, options) => {
        const message = {
            image: { url: image }, ...options,
  ...
  * @param {Object} options 
  */
     conn.sendTBI2 = async (jid, contentText, footer, image, dtux, urlx, buttons1, row1, buttons2, row2, buttons3, row3, quoted, options) => {
        const message = {
            image: { url: image }, ...options,
            jpegThumbnail: await (await fetch(thumbt)).buffer(),
            caption: contentText,
  ...
  */
    ...
    /**
       ...
       * @param {Object} options 
       */
    conn.sendTBVG = async (jid, contentText, footer, video, dtux, urlx, buttons1, row1, buttons2, row2,quoted, options) => {
        const message = {
            video: { url: video }, ...options,
...
exports.smsg = (conn, m, hasParent) => {
    ...
    if (m.message) {
        ...
        if (m.quoted) {
          ...
          //  m.quoted.reply2 = (text, chatId, options) => conn.reply2(chatId ? chatId : m.chat, text, vM, options)
