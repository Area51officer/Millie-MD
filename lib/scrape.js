    ...
    return new Promise((resolve, reject) => {
        fetch('https://jagokata.com/kata-bijak/kata-' + input.replace(/\s/g, '_') + '.html?page=1')
            ...
            .then(res => {
                ...
                data.splice(2, 1)
                if (data.length == 0) return resolve({ creator: 'ɴᴇᴇʀᴀᴊ-x0', status: false })
                resolve({ creator: 'ɴᴇᴇʀᴀᴊ-x0', status: true, data })
            }).catch(reject)
    })
}
...
function facebook2(urls) {
    return new Promise(async (resolve, reject) => {
       ...
       resolve({ status: getPost.status, creator: 'ɴᴇᴇʀᴀᴊ-x0', hasil: hasil })
    })
}
